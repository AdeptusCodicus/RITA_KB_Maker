...
export function saveDraft(draft: Omit<KBDraft, 'id' | 'updatedAt'> & { id?: string }): KBDraft {
  ...
  // Safe save with QuotaExceeded fallback protection
  try {
    localStorage.setItem(DRAFTS_KEY, JSON.stringify(drafts));
  } catch (err) {
    console.warn('LocalStorage quota warning, trimming heavy extractedText on older drafts:', err);
    // Prune raw OCR/extractedText from older drafts to keep metadata and markdown intact
    const trimmedDrafts = drafts.map((d, index) => {
      if (index === 0) return d;
      return {
        ...d,
        extractedText: d.extractedText.length > 500 ? d.extractedText.slice(0, 500) + '... [truncated]' : d.extractedText,
      };
    });
    ...
    try {
      ...
      // If still exceeding, keep only the latest 10 drafts
      const minimalist = trimmedDrafts.slice(0, 10);
      localStorage.setItem(DRAFTS_KEY, JSON.stringify(minimalist));
    }
  }
...
export function deleteDraft(id: string): void {
  ...
  try {
    ...
    console.error('Failed to update drafts in localStorage:', err);
  }
